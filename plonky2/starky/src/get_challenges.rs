...
fn get_challenges<F, C, S: Stark<F, D>, const D: usize>(
    ...
    preprocessed_digest: Option<&HashOut<F>>,
    ...
    C: GenericConfig<D, F = F>,
{
    ...
    // Observe the preprocessed digest first: it identifies the "program" side of the statement
    // alongside the public inputs, and must be absorbed before any challenges are derived.
    if let Some(digest) = preprocessed_digest {
...
fn get_challenges_target<F, C, S: Stark<F, D>, const D: usize>(
    ...
    preprocessed_digest: Option<&[Target]>,
    ...
    C::Hasher: AlgebraicHasher<F>,
{
    ...
    // alongside the public inputs, and must be absorbed before any challenges are derived.
    if let Some(digest) = preprocessed_digest {
        challenger.observe_elements(digest);
    }
