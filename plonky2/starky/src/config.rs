...
pub struct StarkConfig {
    ...
    /// Indices of columns the verifier and prover must agree upon.
    pub preprocessed_columns: SortedSet<usize>,
}
...
impl StarkConfig {
    ...
    pub const fn new(security_bits: usize, num_challenges: usize, fri_config: FriConfig) -> Self {
        Self {
            ...
            num_challenges,
            fri_config,
            preprocessed_columns: SortedSet::new(),
        }
    }
        ...
        H: AlgebraicHasher<F>,
    {
        ...
        // Include length as first element for state separation
        let mut prep_cols =
            vec![builder.constant(F::from_canonical_usize(self.preprocessed_columns.len()))];
        prep_cols.extend(
            self.preprocessed_columns
                .iter()
