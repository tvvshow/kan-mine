...
mod tests {
    ...
    fn test_fibonacci_stark() -> Result<()> {
        ...
        verify_stark_proof(stark, proof, &config, None, None)
    }
    ...
    fn test_recursive_stark_verifier() -> Result<()> {
        ...
        verify_stark_proof(stark, proof.clone(), &config, None, None)?;
        ...
        InnerC::Hasher: AlgebraicHasher<F>,
    {
        ...
        set_stark_proof_with_pis_target(&mut pw, &pt, &inner_proof, degree_bits)?;

        let _ = verify_stark_proof_circuit::<F, InnerC, S, D>(
            &mut builder,
            stark,
            &pt,
            inner_config,
    ...
    fn test_recursive_verifier_with_multiple_degree_bits() -> Result<()> {
        ...
        let _ = verify_stark_proof_circuit::<F, C, S, D>(
            &mut builder,
            stark,
            &pt,
            &stark_config,
        ...
        degree_bits
            ...
            .try_for_each(|(degree_bits, proof)| {
                let mut pw = PartialWitness::new();
                set_stark_proof_with_pis_target(&mut pw, &pt, &proof, degree_bits)?;
                let proof = data.prove(pw)?;
                data.verify(proof)
            })?;
