...
///
/// If `preprocessed_digest` is non-empty, it is absorbed into the challenger,
/// binding the challenge point `zeta` to the preprocessed data.
/// Must be empty or exactly `NUM_HASH_OUT_ELTS` elements.
/// Like [`prove`] but also returns the challenge point `zeta`.
pub fn prove_and_get_zeta<F, C, S, const D: usize>(
    stark: S,
    config: &StarkConfig,
    ...
    preprocessed_digest: &[F],
) -> Result<(StarkProofWithPublicInputs<F, C, D>, F::Extension)>
where
    ...
    S: Stark<F, D>,
{
        ...
        trace_poly_values.len() == S::COLUMNS,
        "Trace poly values length mismatch"
    );
    ensure!(
        preprocessed_digest.is_empty() || preprocessed_digest.len() == NUM_HASH_OUT_ELTS,
        "preprocessed_digest must be empty or exactly {} elements",
        NUM_HASH_OUT_ELTS
    );
    ...
    let (final_poly_coeff_len, max_num_query_steps) =
        if let Some(vp) = verifier_circuit_fri_params.as_ref() {
            assert_eq!(vp.config, fri_params.config);
            let (len, steps) = extract_fri_shape(vp, &config.fri_config.reduction_strategy);
            (Some(len), Some(steps))
        } else {
            (None, None)
        };

    let sparse_trace = create_sparse_trace_for_lookups(&stark.lookups(), &trace_poly_values);

    // Commit all trace columns (online + preprocessed) in a single Merkle tree.
    ...
    if !preprocessed_digest.is_empty() {
        challenger.observe_elements(preprocessed_digest);
    }
    ...
    // Drop large structures in parallel to avoid blocking
    rayon::spawn(move || {
        drop(sparse_trace);
        drop(trace_commitment);
    });
    ...
    S: Stark<F, D>,
{
    ...
    let lookup_helper_columns = timed!(
        ...
        lookup_challenges.as_ref().map(|challenges| {
            lookups
                .iter()
                .flat_map(|lookup| challenges.iter().map(move |&challenge| (lookup, challenge)))
                .collect::<Vec<_>>()
                .into_par_iter()
    ...
    let total_num_dummy_extension_evals = 2 * (S::COLUMNS + num_auxiliary_polys); // twice for zeta and g * zeta
    ...
    let mut initial_merkle_trees = vec![trace];
    for commitment in auxiliary_polys_commitment
        .as_ref()
        .iter()
        .chain(quotient_commitment.as_ref().iter())
    {
        initial_merkle_trees.push(commitment);
    }
    ...
    let proof = StarkProof {
        trace_cap: trace.merkle_tree.cap.clone(),
        auxiliary_polys_cap,
        quotient_polys_cap,
        openings,
        opening_proof,
...
/// Prove a STARK statement. For callers that also need the Fiat-Shamir challenge point `zeta`,
/// use [`prove_and_get_zeta`] instead to avoid re-running the transcript.
    ...
    S: Stark<F, D>,
{
    ...
    // Retrieve the LDE values at index `i` from the single trace commitment.
    let get_trace_values_packed =
        |i_start| -> Vec<P> { trace.get_lde_values_packed(i_start, step) };
    ...
    S: Stark<F, D>,
{
    ...
    let trace_subgroup_evals = transpose(&get_subgroup_evals(trace));
    let auxiliary_subgroup_evals = auxiliary_commitment
        .as_ref()
        .map(|aux| transpose(&get_subgroup_evals(aux)));
...
/// Extract FRI proof shape parameters from verifier circuit FRI params.
/// Returns (final_poly_coeff_len, max_num_query_steps).
fn extract_fri_shape(vp: &FriParams, reduction_strategy: &FriReductionStrategy) -> (usize, usize) {
    match reduction_strategy {
        FriReductionStrategy::ConstantArityBits(_, final_poly_bits) => {
            let len = final_poly_coeff_len(vp.degree_bits, &vp.reduction_arity_bits);
            assert_eq!(len, 1 << (1 + *final_poly_bits));
            (len, vp.reduction_arity_bits.len())
        }
        _ => panic!("FRI Reduction Strategy is not ConstantArityBits"),
    }
}

/// Create a sparse trace containing only lookup-relevant columns.
/// Non-lookup columns are replaced with minimal placeholders to save memory.
fn create_sparse_trace_for_lookups<F: Field>(
    lookups: &[Lookup<F>],
    trace: &[PolynomialValues<F>],
) -> Vec<PolynomialValues<F>> {
    let mut lookup_indices: std::collections::HashSet<usize> = lookups
        .iter()
        .flat_map(|l| l.all_column_indices())
        .collect();
    lookup_indices.insert(0); // Always include column 0 for degree
