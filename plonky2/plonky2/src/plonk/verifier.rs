...
>(
    ...
    let (zeta_pow_deg_for_reducing, chunk_size) = if common_data.config.zero_knowledge {
        // In the zk case, the quotient chunk size is smaller.
        // This means the power of zeta for reducing the quotient chunks is also smaller.
        let h_p = common_data.quotient_chunk_blinding_degree();
        let chunk_deg = common_data.degree() - h_p;
        let chunk_size =
            (common_data.quotient_degree_factor * common_data.degree()).div_ceil(chunk_deg);
        (challenges.plonk_zeta.exp_u64(chunk_deg as u64), chunk_size)
    } else {
        (
            challenges
                .plonk_zeta
