...
use crate::plonk::circuit_data::{
    quotient_chunk_blinding_degree, witness_polynomial_blinding_degree, CircuitConfig, CircuitData,
    CommonCircuitData, MockCircuitData, ProverCircuitData, ProverOnlyCircuitData,
...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
    ...
    fn blind_and_pad(&mut self) {
        if self.config.zero_knowledge {
            ...
            // opened at two).
            let blinding_degree = witness_polynomial_blinding_degree::<D>(&self.config.fri_config);
            debug!("Adding {blinding_degree} blinding terms for witness polynomials || Original len: {}", self.gate_instances.len());

            // Add regular blinding gates
        ...
        let mut min_degree = self.gate_instances.len().next_power_of_two();
        if self.config.zero_knowledge {
            while min_degree
                < 2 * quotient_chunk_blinding_degree::<D>(
                    &self.config.fri_config,
                    &self.fri_params(log2_strict(min_degree)),
                )
            {
                min_degree *= 2;
            }
        }
        while self.gate_instances.len() < min_degree {
            self.add_gate(NoopGate, vec![]);
        }
    }

    /// Add a single regular blinding gate (adds 1 gate)
    fn add_regular_blinding(&mut self) {
    ...
    fn sigma_vecs(&self, k_is: &[F], subgroup: &[F]) -> (Vec<PolynomialValues<F>>, Forest) {
        ...
        #[cfg(debug_assertions)]
        {
            ...
            for gate in 0..degree {
                for input in 0..config.num_wires {
                    ...
                    assert_eq!(forest.target_index(t), target_idx);
                    target_idx += 1;
                }
            }
        ...
        let sigma_polys = forest.get_sigma_polys(k_is, subgroup);
        (sigma_polys, forest)
    }
