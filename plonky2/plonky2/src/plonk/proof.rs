...
/// A proof with public inputs that was serialized in compact format.
///
/// Structurally identical to [`ProofWithPublicInputs`], but the compact format omits
/// Tree 0 (constants_sigmas) evaluations and Merkle proofs from FRI query rounds.
/// [`verify`](Self::verify) reconstructs the omitted Tree 0 data from trusted
/// polynomial coefficients before delegating to the standard verification path.
#[derive(Debug)]
pub struct CompactProofWithPublicInputs<
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F>,
    const D: usize,
> {
...
impl<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>
    CompactProofWithPublicInputs<F, C, D>
{
    /// Serialize the proof in compact format, omitting Tree 0 (constants_sigmas)
    /// evaluations and Merkle proofs from FRI query rounds.
    pub fn to_proof_bytes(&self) -> Vec<u8> {
        let mut buffer = Vec::new();
        buffer
            .write_proof::<F, C, D>(&self.proof, &[PlonkOracle::CONSTANTS_SIGMAS.index])
            .expect("Writing to a byte-vector cannot fail.");
        buffer
    }

    /// Deserialize a compact proof and attach public inputs.
    ///
    /// The deserialized proof has dummy Tree 0 entries. Call [`verify`](Self::verify)
    /// to fill these from polynomial coefficients and verify the proof.
    pub fn from_bytes(
        bytes: &[u8],
        public_inputs: Vec<F>,
        common_data: &CommonCircuitData<F, D>,
        ...
        let mut buffer = Buffer::new(bytes);
        let proof =
            buffer.read_proof::<F, C, D>(common_data, &[PlonkOracle::CONSTANTS_SIGMAS.index])?;
        // Ensure all bytes were consumed
        if !buffer.unread_bytes().is_empty() {
            return Err(IoError);
        }
        Ok(Self {
            proof,
    ...
    /// Verify the compact proof, filling in omitted Tree 0 data from polynomial coefficients.
    pub fn verify(
        self,
        verifier_data: &VerifierOnlyCircuitData<C, D>,
        common_data: &CommonCircuitData<F, D>,
        ...
        let public_inputs_hash = C::InnerHasher::hash_no_pad(&self.public_inputs);
        let mut proof = self.proof;
        // This closure fills proof.openings.{constants, plonk_sigmas} as a side effect so
        // that verify_with_challenges (called below) sees the populated fields directly.
        let openings_at_zeta = |zeta| {
            Self::fill_constants_sigmas_openings(
                &mut proof.openings,
                zeta,
                constants_sigmas_polynomials,
                common_data,
            );
            proof.openings.clone()
        };
        let challenges = get_challenges::<F, C, D, _>(
            public_inputs_hash,
            &proof.wires_cap,
        ...
        Self::fill_constants_sigmas_evals(
            &mut proof,
            &challenges,
            constants_sigmas_polynomials,
            common_data,
    ...
    fn fill_constants_sigmas_openings(
        openings: &mut OpeningSet<F, D>,
        zeta: F::Extension,
        constants_sigmas_polynomials: &[PolynomialCoeffs<F>],
        common_data: &CommonCircuitData<F, D>,
    ) {
    ...
    fn fill_constants_sigmas_evals(
        proof: &mut Proof<F, C, D>,
        challenges: &ProofChallenges<F, D>,
        constants_sigmas_polynomials: &[PolynomialCoeffs<F>],
        common_data: &CommonCircuitData<F, D>,
    ) {
        let lde_bits = common_data.fri_params.lde_bits();
        let points: Vec<F> = challenges
            .fri_challenges
            .fri_query_indices
            .iter()
            .map(|&x| lde_coset_point(x, lde_bits))
            .collect();
        let constants_sigmas_evals =
            evaluate_polynomials_at_points(constants_sigmas_polynomials, &points);
        for (round, evals) in proof
            .opening_proof
            .query_round_proofs
            .iter_mut()
            .zip(constants_sigmas_evals)
        {
            round.initial_trees_proof.evals_proofs[PlonkOracle::CONSTANTS_SIGMAS.index].0 = evals;
        }
    }
}

impl<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>
    From<ProofWithPublicInputs<F, C, D>> for CompactProofWithPublicInputs<F, C, D>
{
    fn from(p: ProofWithPublicInputs<F, C, D>) -> Self {
        Self {
            proof: p.proof,
