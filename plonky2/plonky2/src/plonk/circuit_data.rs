    ...
    MockCircuitData<F, C, D>
{
    pub fn generate_witness(&self, inputs: PartialWitness<F>) -> PartitionWitness<'_, F> {
        generate_partial_witness::<F, C, D>(inputs, &self.prover_only, &self.common, false)
...
> {
    ...
    /// Precomputed topological order of generators, grouped into layers.
    /// Generators within each layer can run in parallel. Computed on first proof generation.
    pub layered_generators: Vec<Vec<u32>>,
}
...
/// Computes the value of `h`, corresponding to the the witness polynomials randomization
/// degree, as defined in equation (10) of https://eprint.iacr.org/2024/1037.pdf
/// D is the degree extension
pub fn witness_polynomial_blinding_degree<const D: usize>(fri_config: &FriConfig) -> usize {
    let num_query_rounds = fri_config.num_query_rounds;
    // Two openings (x, g*x) per initial step of a FRI round
    // Two openings in DEEP
    // Upper bound on number of opening at FRI initial step
    2 * D * (num_query_rounds + 1) * 2
}

/// Computes the value of `h_p`, corresponding to the quotient chunks randomization
/// degree, as defined in equation (9) of https://eprint.iacr.org/2024/1037.pdf
pub fn quotient_chunk_blinding_degree<const D: usize>(
    fri_config: &FriConfig,
    fri_params: &FriParams,
) -> usize {
    let fri_openings_per_round: usize = fri_params
        .reduction_arity_bits
        .iter()
        .map(|x| (1 << x) - 1)
        .sum();
    let arities_product = 1usize << fri_params.reduction_arity_bits.iter().sum::<usize>();
    let final_poly_coeffs = (1 << fri_params.degree_bits) / arities_product;
    // Upper bound on total number of opened values in entire FRI
    2 * D * (fri_config.num_query_rounds * fri_openings_per_round + final_poly_coeffs + 1)
}
...
impl<F: RichField + Extendable<D>, const D: usize> CommonCircuitData<F, D> {
    ...
    pub fn quotient_range(&self) -> Range<usize> {
    ...
    /// Computes the value of `h_p`, corresponding to the the quotient chunks randomization
    ///  degree, as defined in equation (9) of https://eprint.iacr.org/2024/1037.pdf
    pub fn quotient_chunk_blinding_degree(&self) -> usize {
        if !self.config.zero_knowledge {
            return 0;
        }
        quotient_chunk_blinding_degree::<D>(&self.config.fri_config, &self.fri_params)
    }

    pub(crate) fn num_quotient_polys(&self) -> usize {
        if !self.config.zero_knowledge {
            return self.config.num_challenges * self.quotient_degree_factor;
        }
        // In the case of zk, the chunks of the quotient polynomials get smaller.
        // h_p as in equation (9) in https://eprint.iacr.org/2024/1037.pdf
        let h_p = self.quotient_chunk_blinding_degree();
        let chunk_deg = self.degree().saturating_sub(h_p);
        assert!(chunk_deg > 0);

        let num_randomized_chunks = self.quotient_degree().div_ceil(chunk_deg);
        self.config.num_challenges * num_randomized_chunks
    }
