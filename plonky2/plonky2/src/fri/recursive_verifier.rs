...
use crate::fri::structure::{
    FriBatchInfoTarget, FriInstanceInfoTarget, FriOpeningsTarget, FriPolynomialInfo, ZETA_BATCH_IDX,
};
...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
    ...
    fn fri_combine_initial(
            ...
            .enumerate()
        {
            ...
            let num_r_polys = (params.hiding && idx == ZETA_BATCH_IDX) as usize;
            let last_poly = polynomials.len() - num_r_polys;
            let eval = |p: &FriPolynomialInfo| {
            ...
            let evals = polynomials[..last_poly].iter().map(eval).collect_vec();
            ...
            if num_r_polys > 0 {
                ...
                let reduced_evals = alpha.reduce_base(&evals, self);
                sum = alpha.shift(sum, self);
                sum = self.add_extension(reduced_evals, sum);
            }
        }
...
impl<const D: usize> PrecomputedReducedOpeningsTarget<D> {
    pub(crate) fn from_os_and_alpha<F: RichField + Extendable<D>>(
        ...
        let reduced_openings_at_point = openings
            ...
            .enumerate()
            .map(|(idx, batch)| {
                let num_r_polys = (is_zk && idx == ZETA_BATCH_IDX) as usize;
                ReducingFactorTarget::new(alpha)
                    .reduce(&batch.values[..batch.values.len() - num_r_polys], builder)
            })
            .collect();
        Self {
