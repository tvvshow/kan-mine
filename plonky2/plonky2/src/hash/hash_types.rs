...
use crate::plonk::config::{AlgebraicHasher, GenericConfig, GenericHashOut};

/// A prime order field with the features we need to use it as a base field in our argument system.
pub trait RichField: PrimeField64 + Poseidon + 'static {}
...
impl MerkleCapTarget {
    pub fn digest<F, C, const D: usize>(&self, builder: &mut CircuitBuilder<F, D>) -> HashOutTarget
    where
        ...
        C::Hasher: AlgebraicHasher<F>,
    {
        ...
        challenger.observe_element(builder.constant(F::from_canonical_usize(self.0.len())));
        challenger.observe_cap(self);
        challenger.get_hash(builder)
    }
}
