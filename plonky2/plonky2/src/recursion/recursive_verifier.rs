...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
        ...
        C::Hasher: AlgebraicHasher<F>,
    {
        ...
        let (zeta_pow_deg_for_reducing, chunk_size) = if inner_common_data.config.zero_knowledge {
            ...
            // is also smaller.
            let h_p = inner_common_data.quotient_chunk_blinding_degree();
            let chunk_deg = inner_common_data.degree() - h_p;
            let chunk_size = (inner_common_data.quotient_degree_factor
                * inner_common_data.degree())
            .div_ceil(chunk_deg);
            (
                self.exp_u64_extension(challenges.plonk_zeta, chunk_deg as u64),
                chunk_size,
            )
        } else {
            (
