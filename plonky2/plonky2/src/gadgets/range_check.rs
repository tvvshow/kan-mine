...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
    ...
    pub fn range_check(&mut self, x: Target, n_log: usize) -> Vec<BoolTarget> {
    ...
    pub fn split_low_high(&mut self, x: Target, n_log: usize, num_bits: usize) -> (Target, Target) {
        let (low, high, _, _) = self.split_low_high_keep_bits(x, n_log, num_bits);
        (low, high)
    }

    /// ditto to split_low_high but returns a,b, a as bits, b as bits
    pub fn split_low_high_keep_bits(
        &mut self,
        x: Target,
        n_log: usize,
        num_bits: usize,
    ) -> (Target, Target, Vec<BoolTarget>, Vec<BoolTarget>) {
        ...
        let bits_low = self.range_check(low, n_log);
        let bits_high = self.range_check(high, num_bits - n_log);

        let pow2 = self.constant(F::from_canonical_u64(1 << n_log));
        let comp_x = self.mul_add(high, pow2, low);
        self.connect(x, comp_x);

        (low, high, bits_low, bits_high)
    }
