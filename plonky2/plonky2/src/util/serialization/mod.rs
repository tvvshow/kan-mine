...
pub trait Read {
    ...
    /// Reads `num_bytes` bytes and interprets them as a little-endian `usize`.
    #[inline]
    fn read_uint_le(&mut self, num_bytes: usize) -> IoResult<usize> {
        let mut buf = [0u8; size_of::<usize>()];
        self.read_exact(&mut buf[..num_bytes])?;
        ...
        C: GenericConfig<D, F = F>,
    {
        ...
        let (constants, plonk_sigmas) =
            if oracles_to_skip.contains(&PlonkOracle::CONSTANTS_SIGMAS.index) {
                (
                    vec![F::Extension::default(); common_data.num_constants],
                    vec![F::Extension::default(); config.num_routed_wires],
                )
            } else {
                (
                    self.read_field_ext_vec::<F, D>(common_data.num_constants)?,
                    self.read_field_ext_vec::<F, D>(config.num_routed_wires)?,
                )
            };
    ...
    /// Reads a value of type [`FriInitialTreeProof`] from `self`, skipping oracle data at specified indices.
        ...
        let oracle_sizes = [
            ...
            config.num_challenges
                * (1 + common_data.num_partial_products + common_data.num_lookup_polys)
                + salt,
            common_data.num_quotient_polys() + common_data.num_r_polys() + salt,
        ];

        let mut evals_proofs = Vec::with_capacity(oracle_sizes.len());
        for (i, &size) in oracle_sizes.iter().enumerate() {
            if oracles_to_skip.contains(&i) {
                evals_proofs.push((vec![], MerkleProof { siblings: vec![] }));
            } else {
                let v = self.read_field_vec(size)?;
                let p = self.read_merkle_proof()?;
                evals_proofs.push((v, p));
            }
        }
    ...
    /// Reads a vector of [`FriQueryRound`]s from `self`, skipping oracle data at specified indices.
        ...
        C: GenericConfig<D, F = F>,
    {
        ...
        for _ in 0..config.fri_config.num_query_rounds {
            let initial_trees_proof =
                self.read_fri_initial_proof::<F, C, D>(common_data, oracles_to_skip)?;
    ...
    /// Reads a value of type [`FriProof`] from `self`, skipping oracle data at specified indices.
        ...
        C: GenericConfig<D, F = F>,
    {
        ...
        let query_round_proofs =
            self.read_fri_query_rounds::<F, C, D>(common_data, oracles_to_skip)?;
    ...
    /// Reads a value of type [`Proof`] from `self`, skipping oracle data at specified indices.
        ...
        C: GenericConfig<D, F = F>,
    {
        ...
        let openings = self.read_opening_set::<F, C, D>(common_data, oracles_to_skip)?;
        let opening_proof = self.read_fri_proof::<F, C, D>(common_data, oracles_to_skip)?;
        Ok(Proof {
            wires_cap,
...
pub trait Write {
    ...
    /// Writes an [`OpeningSet`], optionally skipping oracle data at specified indices.
    ///
    /// Oracle 0 maps to `constants` + `plonk_sigmas`. Skipping oracle 0 omits those fields.
    ...
    /// Writes an [`FriInitialTreeProof`], skipping oracle data at specified indices.
    ...
    where
        ...
        C: GenericConfig<D, F = F>,
    {
        for (i, (v, p)) in fitp.evals_proofs.iter().enumerate() {
            if !oracles_to_skip.contains(&i) {
    ...
    /// Writes [`FriQueryRound`]s, skipping oracle data at specified indices.
    ...
    {
        for fqr in fqrs {
            self.write_fri_initial_proof::<F, C, D>(&fqr.initial_trees_proof, oracles_to_skip)?;
    ...
    /// Writes a [`FriProof`], skipping oracle data at specified indices.
        ...
        C: GenericConfig<D, F = F>,
    {
        ...
        self.write_fri_query_rounds::<F, C, D>(&fp.query_round_proofs, oracles_to_skip)?;
    ...
    /// Writes a [`Proof`], skipping oracle data at specified indices.
        ...
        C: GenericConfig<D, F = F>,
    {
        ...
        self.write_opening_set::<F, D>(&proof.openings, oracles_to_skip)?;
        self.write_fri_proof::<F, C, D>(&proof.opening_proof, oracles_to_skip)
    }
...
impl<'a> Buffer<'a> {
    ...
    /// Reads a zero-copy slice of `n` bytes, advancing the position.
    #[inline]
    pub fn read_bytes(&mut self, n: usize) -> IoResult<&'a [u8]> {
        let start = self.pos;
        let end = start
            .checked_add(n)
