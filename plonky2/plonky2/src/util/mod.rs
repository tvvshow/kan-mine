...
pub(crate) fn evaluate_polynomials_at_points<F: Field>(
    polynomials: &[PolynomialCoeffs<F>],
    points: &[F],
) -> Vec<Vec<F>> {
    if polynomials.is_empty() {
        return vec![vec![]; points.len()];
    }
...
/// Returns the field element at position `x_index` in the LDE coset domain of size `2^lde_bits`.
///
/// The domain is the coset `g * <ω>` where `g` is the multiplicative generator and `ω` is a
/// primitive `2^lde_bits`-th root of unity, laid out in bit-reversed order.
pub(crate) fn lde_coset_point<F: RichField>(x_index: usize, lde_bits: usize) -> F {
    F::MULTIPLICATIVE_GROUP_GENERATOR
        * F::primitive_root_of_unity(lde_bits).exp_u64(reverse_bits(x_index, lde_bits) as u64)
}
