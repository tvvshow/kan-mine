...
use plonky2_maybe_rayon::rayon::iter::ParallelIterator;
use plonky2_maybe_rayon::rayon::slice::ParallelSlice;
...
/// given set of generators. If `track_order` is true, also returns the order in which generators finished,
/// grouped into parallelizable layers (used by `generate_partial_witness_fast`).
...
>(
    ...
    track_order: bool,
) -> Result<(PartitionWitness<'a, F>, Vec<Vec<u32>>)> {
    ...
    // Order in which generators finish, grouped into layers (for parallelization)
    ...
    while !pending_generator_indices.is_empty() {
        ...
        for &generator_idx in &pending_generator_indices {
            ...
            if finished {
                ...
                if track_order {
                    current_layer.push(generator_idx as u32);
                }
            }
        }

        // Merge generated values into our witness, and enqueue unfinished generators
        // that were watching one of the newly populated targets.
        for (t, v) in buffer.target_values.drain(..) {
            let reps = witness.set_target_returning_rep(t, v)?;
            if let Some(watch) = reps {
        ...
        if track_order && !current_layer.is_empty() {
            current_layer.shrink_to_fit();
            order.push(current_layer);
        }
...
/// Parallel witness generation using precomputed topological order.
/// Process `layered_generators` in parallel within each layer.
pub fn generate_partial_witness_fast<
    'a,
    F: RichField + Extendable<D>,
    C: GenericConfig<D, F = F>,
    const D: usize,
>(
    ...
    let config = &common_data.config;
    let generators = &prover_data.generators;
    let layers = &prover_data.layered_generators;
    ...
    let values_ptr = AtomicPtr::new(witness.values.as_mut_ptr());

    for layer in layers {
        // Each thread processes a chunk of generators
        layer
            .par_chunks(4)
            .for_each_init(GeneratedValues::empty, |buffer, chunk| {
                let ptr = values_ptr.load(Ordering::Relaxed);
                for &generator_idx in chunk {
                    // SAFETY: generator_idx comes from layers which contain valid indices
                    let finished = unsafe { generators.get_unchecked(generator_idx as usize) }
                        .0
                        .run(&witness, buffer);
                    debug_assert!(finished);

                    // SAFETY: Each generator produces unique targets, so writes are disjoint.
                    for (t, v) in buffer.target_values.drain(..) {
                        unsafe { witness.set_target_unsafe(ptr, t, v) };
                    }
                }
            });
    }
