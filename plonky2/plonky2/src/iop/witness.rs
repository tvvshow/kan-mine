...
impl<'a, F: Field> PartitionWitness<'a, F> {
    ...
    pub fn set_target_returning_rep(&mut self, target: Target, value: F) -> Result<Option<usize>> {
        ...
        if rep_value.is_some() {
            #[cfg(debug_assertions)]
            if rep_value.unwrap() != value {
                return Err(anyhow!(
    ...
    /// Set a target value using a raw pointer to the values array.
    ///
    /// # Safety
    /// - `values_ptr` must point to `self.values.as_mut_ptr()`
    /// - Caller must ensure no data races (e.g., different threads write to different targets)
    #[inline]
    pub unsafe fn set_target_unsafe(&self, values_ptr: *mut Option<F>, target: Target, value: F) {
        let target_index = self.target_index(target);
        let rep_index = *self.representative_map.get_unchecked(target_index);
        *values_ptr.add(rep_index) = Some(value);
    }
