...
impl Sample for GoldilocksField {
    ...
    fn rand() -> Self {
        ...
        std::thread_local! {
            static CACHE: RefCell<[GoldilocksField; SZ]> = const { RefCell::new([GoldilocksField::ZERO; SZ]) };
            static INDEX: Cell<usize> = const { Cell::new(SZ) };
        }
        ...
        CACHE.with(|cache| {
            INDEX.with(|index| {
                ...
                if idx >= SZ {
                    *cache.borrow_mut() = Self::rand_vec(SZ).try_into().unwrap();
                    idx = 0;
                }
                index.set(idx + 1);
                cache.borrow()[idx]
