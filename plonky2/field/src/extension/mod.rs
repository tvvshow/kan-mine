...
pub trait Extendable<const D: usize>: Field + Sized {
    ...
    /// Multiply two extension field elements represented as arrays of base field elements.
    /// The default performs generic polynomial multiplication modulo `X^D - W`.
    /// Concrete fields (e.g. Goldilocks) override this with optimised implementations.
    fn ext_mul(a: [Self; D], b: [Self; D]) -> [Self; D] {
        let w = Self::W;
...
impl<F: Field + Frobenius<1> + FieldExtension<1, BaseField = F>> Extendable<1> for F {
    ...
    #[inline]
    fn ext_mul(a: [Self; 1], b: [Self; 1]) -> [Self; 1] {
        [a[0] * b[0]]
    }
}
