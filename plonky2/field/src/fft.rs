...
fn fft_classic_simd<P: PackedField>(
    ...
    let minimal_step = 1 << (s + 1 - lg_packed_width);
    for packed_end in (minimal_step..(1 + packed_n)).step_by(minimal_step) {
        for lg_half_m in s..lg_n {
            let half_packed_m = 1 << (lg_half_m - lg_packed_width);
            debug_assert!(half_packed_m != 0);
            if (packed_end & half_packed_m) != 0 {
                break;
            }
            let omega_table = P::pack_slice(&root_table[lg_half_m][..]);
            let k = packed_end - (half_packed_m << 1);
            let kk = k + half_packed_m;

            for j in 0..half_packed_m {
                let omega = omega_table[j];
                let t = omega * packed_values[kk + j];
