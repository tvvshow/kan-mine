#!/usr/bin/env bash
# Phase A1 (fold direct-store) A/B + bit-exactness gate.
#
...
#
# For the REAL config the two MUST be byte-identical (the winning proof's
# sha256 must match); A1 is purely a strength-reduction of an algebraically
# equal expression. The script FAILS loudly if they diverge.
#
...
run_variant() {  # $1=label  $2=NVCC_EXTRA
  ...
  # --- correctness + bit-exact proof: easy target -> draw-1 winner -> POSTCHECK ---
  $PG --cfg real --mine 1 --target "$EASY" "$SEED" >/tmp/a1_proof_$label.b64 2>/tmp/a1_corr_$label.log
  local postck; postck=$(grep -oE "POSTCHECK ok=[01]" /tmp/a1_corr_$label.log | tail -1)
  local sha;    sha=$(sha256sum /tmp/a1_proof_$label.b64 | awk '{print $1}')
  echo "  correctness: ${postck:-<none>}   proof.sha256=${sha:0:16}…"
...
echo "  proof sha  RMW=${SHA_RMW:0:16}…  A1=${SHA_A1:0:16}…"

fail=0
[ "$OK_RMW" = "POSTCHECK ok=1" ] || { echo "  ✗ RMW POSTCHECK not ok=1"; fail=1; }
[ "$OK_A1"  = "POSTCHECK ok=1" ] || { echo "  ✗ A1  POSTCHECK not ok=1"; fail=1; }
if [ "$SHA_RMW" != "$SHA_A1" ]; then
  echo "  ✗ PROOF DIVERGED — A1 is NOT byte-identical to RMW for REAL config. REVERT A1."
  fail=1
else
  echo "  ✓ proof byte-identical (A1 == RMW) for REAL config"
fi
...
if [ "$fail" = 0 ]; then
  ...
  echo ""
  echo "  GATE: deploy A1 if Δ stably positive AND proof identical AND no new ptxas spill."
fi
exit $fail
